...
public:
	...
	enum AnimationID
	{
		...
		StandAfterBreak = 21,   // 急ブレーキ後の立上り用 - スマブラのファルコンのモーションをトレス
		Standing2 = 22,         // 立ちポーズ - スマブラのファルコンのモーションをトレス
		Running2 = 23,          // 走りモーション用 - スマブラのファルコンのモーションをトレス
		// -- 2018/05/05 走り出しモーションの追加と、切返しなし急ブレーキのモーションを走り止まりモーションに差し替え
		BreaktoStop2 = 24,      // 急ブレーキ（そのまま停止）用 - スマブラのファルコンのモーションをトレス
		StandAfterBreak2 = 25,  // 急ブレーキ後の立上り用 - スマブラのファルコンのモーションをトレス
		StartRunning = 26,      // 走り出しモーション - スマブラのファルコンのモーションをトレス
		StartWalking = 27,      // 歩き出しモーション - スマブラのファルコンのモーションをトレス
		BreaktoTurn_R = 28,     // BreaktoTurnの左右反転
		Turn_R = 29,            // Turnの左右反転
		BreaktoStop2_R = 30,    // BreaktoStop2の左右反転
		StandAfterBreak2_R = 31 // StandAfterBreak2の左右反転

	};

	static const int m_iAnimIDMax=32;
